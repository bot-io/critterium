...
- **9 force types via dynamic force registry** — drag, wander, gravity, flow fields, vortices, pointers, alignment, boids, and attractors. Add, remove, and tune forces at runtime from the UI — no code changes needed.
- **14 built-in presets** — curated ecosystems with emergent behavior, from starling murmurations to hydrothermal vents and symbiotic reefs.
...
| **Coral Reef**          | A vibrant reef food chain: Coral feeds Zooplankton → Clownfish → Moray Eels → Reef Sharks, all swept by a gentle turbulence current.                         |
| **Tornado Alley**       | A chaotic vortex sweeps Dust Motes, Debris, and Birds around a swirling core with turbulent flow fields and heavy inward pull.                               |
| **Deep Sea Vent**       | Hydrothermal vent on the ocean floor: Bacteria bloom, Tube Worms filter-feed, Crabs scavenge, and an Octopus hunts — drifting in the vent plume's upwelling. |
| **Symbiosis**           | A peaceful reef of mutual benefit: Algae and Coral share mutual attraction, Cleaner Shrimp tend Coral. No predation — every interaction is positive.         |
...
- **Force pipeline** — a registry-driven pipeline of pluggable forces applied in sequence each step. 9 built-in types, extensible via `registerForceType()`:
  - `PairwiseForce` — N×N interaction matrix (asymmetric, drives chase/flee)
  - `AlignmentForce` — steer toward average neighbor heading (flocking)
  - `BoidsForce` — combined separation + alignment + cohesion (Reynolds flocking)
  - `GlobalForce` — drag, gravity, boundaries (bounce/wrap)
  - `WanderForce` — per-particle smooth noise for organic motion
  - `FlowFieldForce` — spatially varying directional force (currents, wind)
  - `VortexForce` — swirl around a point
  - `AttractorForce` — radial attraction/repulsion toward a point (gravity well)
