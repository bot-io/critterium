...
function ecoCfg(speciesList: SpeciesConfig[], extra: Record<string, any> = {}) {
...
describe('sim: boundaries', () => {
  ...
  it('wrap wraps position', () => {
    ...
    w.y[0] = 310;
    w.applyBoundaries();
    expect(w.x[0]).toBeCloseTo(10, 5);
    expect(w.y[0]).toBeCloseTo(10, 5);
  });
  ...
  it('wrap handles negative positions', () => {
    ...
    w.y[0] = -5;
    w.applyBoundaries();
    expect(w.x[0]).toBeCloseTo(390, 5);
    expect(w.y[0]).toBeCloseTo(295, 5);
  });
});
